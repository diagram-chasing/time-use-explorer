...
async function setupWithSampleData(duckdbInstance: AsyncDuckDB): Promise<void> {
  ...
  try {
    ...
    // Import and register time functions first, BEFORE creating views that use them
    const { queryExecutor } = await import('./queryExecutor');
    await queryExecutor.registerTimeFunctions(duckdbInstance);
    console.log("Registered time functions for calculations");

    // Verify the function exists before using it
    try {
      await conn.query(`SELECT time_diff_minutes('09:00', '10:00') AS test_result`);
      console.log("time_diff_minutes function verified to be working");

      // Create optimized views AFTER the function is registered and verified
      await conn.query(`
      ...
      `);
      console.log("Created optimized views");
    } catch (funcError) {
      console.error("Error with time functions, cannot create views that use them:", funcError);
    }
